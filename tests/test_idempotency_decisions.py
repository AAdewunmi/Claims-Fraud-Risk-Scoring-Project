...
@pytest.mark.django_db
def test_claim_create_idempotency_replay_returns_same_response_and_no_duplicate_audit(api_client):
    ...
    assert AuditEvent.objects.filter(claim_id=first_id, event_type="CLAIM_CREATED").count() == 1
    assert AuditEvent.objects.filter(claim_id=first_id, event_type="SLA_STARTED").count() == 1
...
@pytest.mark.django_db
def test_document_upload_idempotency_replay_returns_same_response_and_no_duplicate_audit(api_client):
    ...
    payload = {"file": uploaded, "original_filename": "photo.jpg", "content_type": "image/jpeg"}

    first = api_client.post(doc_url, data=payload, format="multipart", **headers)
    assert first.status_code == 201
    first_id = first.json()["id"]

    uploaded_again = SimpleUploadedFile("photo.jpg", b"binarydata", content_type="image/jpeg")
    second = api_client.post(
        doc_url,
        data={
            "file": uploaded_again,
    ...
    assert AuditEvent.objects.filter(claim_id=claim.pk, event_type="DOCUMENT_UPLOADED").count() == 1


@pytest.mark.django_db
def test_document_upload_idempotency_conflict_on_payload_mismatch_returns_409(api_client):
    """Same Idempotency-Key with different payload should return 409."""
    ...
    ok = api_client.post(
        doc_url,
        data={"file": uploaded, "original_filename": "photo.jpg", "content_type": "image/jpeg"},
        format="multipart",
        **headers,
    )
    assert ok.status_code == 201

    uploaded_diff = SimpleUploadedFile("photo.jpg", b"different", content_type="image/jpeg")
    conflict = api_client.post(
        doc_url,
        data={"file": uploaded_diff, "original_filename": "photo.jpg", "content_type": "image/jpeg"},
        format="multipart",
        **headers,
    )
    assert conflict.status_code == 409
...
@pytest.mark.django_db
def test_note_create_idempotency_replay_returns_same_response_and_no_duplicate_audit(api_client):
    ...
    first = api_client.post(note_url, data={"body": "Reviewed."}, format="json", **headers)
    assert first.status_code == 201
    first_id = first.json()["id"]

    second = api_client.post(note_url, data={"body": "Reviewed."}, format="json", **headers)
    assert second.status_code == 201
    assert second.json()["id"] == first_id

    assert AuditEvent.objects.filter(claim_id=claim.pk, event_type="NOTE_ADDED").count() == 1


@pytest.mark.django_db
def test_note_create_idempotency_conflict_on_payload_mismatch_returns_409(api_client):
    """Same Idempotency-Key with different payload should return 409."""
    ...
    conflict = api_client.post(note_url, data={"body": "Different"}, format="json", **headers)
    assert conflict.status_code == 409
    assert "detail" in conflict.json()
