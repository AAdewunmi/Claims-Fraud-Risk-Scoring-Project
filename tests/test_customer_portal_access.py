...
def users(customer_group, reviewer_group):
    ...
    c1 = User.objects.create_user(username="cust1", email="cust1@example.com", password="pass-12345-strong")
    c1.groups.add(customer_group)

    c2 = User.objects.create_user(username="cust2", email="cust2@example.com", password="pass-12345-strong")
    c2.groups.add(customer_group)

    reviewer = User.objects.create_user(username="rev", email="rev@example.com", password="pass-12345-strong")
...
def _make_claim_for_policy(policy: Policy, *, idx: int) -> Claim:
...
def test_customer_only_sees_owned_claims_in_list(client, users):
    ...
    _make_claim_for_policy(policy1, idx=1)
    _make_claim_for_policy(policy2, idx=2)
...
def test_customer_cannot_open_other_users_claim_detail(client, users):
    ...
    claim1 = _make_claim_for_policy(policy1, idx=1)
    claim2 = _make_claim_for_policy(policy2, idx=2)
